...
void Texture::render(int x, int y, int ssRow, int ssCol, int w, int h)
{
	...
	SDL_Rect src = this->getSpritePosition (ssRow, ssCol, w, h);
 ...
 * @return An SDL_Rect that can be used by the render function to blit the sprite to the screen
 */
SDL_Rect Texture::getSpritePosition(int ssRow, int ssCol, int width, int height)
{
	SDL_Rect rect = {ssCol*myTileWidth, ssRow*myTileHeight, width*myTileWidth, height*myTileHeight};
	return rect;
}
